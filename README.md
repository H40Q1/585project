...
- We have tried different values for content weights and style weights. For each value we tested, we could get a series value of content loss and style loss which are calculated separately in the relevant layers mentioned in ”Implementation”. When we are considering quantitative evaluation, we could only evaluate the content loss and style loss to interpret the quality of the output images theoretically. The losses are compared every time you have a new style transfer result and we find out that there are significant relations between the total loss, the weighted sum of content loss and style loss. We found out that the final image quality is better when we tried to reduce the total loss. However, we could not discover the direct relationship between the total loss and the quality of an output image. That is because it is very difficult for us to define image quality mathematically, making it hard for us to analyze it quantitatively. Therefore we could only use the total loss to represent the quantitative  evaluation of the final image quality.    
